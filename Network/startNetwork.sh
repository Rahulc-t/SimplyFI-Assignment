...
peer lifecycle chaincode package scholar.tar.gz --path ${PWD}/../Chaincode --lang node --label scholar_1.0
sleep 1

echo "—---------------install chaincode in scholarshipDepartment peer—-------------"

peer lifecycle chaincode install scholar.tar.gz
sleep 3

peer lifecycle chaincode queryinstalled
sleep 1

export CC_PACKAGE_ID=$(peer lifecycle chaincode calculatepackageid scholar.tar.gz)

echo "—---------------Approve chaincode in scholarshipDepartment peer—-------------"

peer lifecycle chaincode approveformyorg -o localhost:7050 --ordererTLSHostnameOverride orderer.scholar.com --channelID $CHANNEL_NAME --name Scholar --version 1.0 --package-id $CC_PACKAGE_ID --sequence 1 --tls --cafile $ORDERER_CA --waitForEvent
sleep 2
...
echo "—---------------Join treasury peer0 to the channel—-------------"
...
echo "—---------------Join treasury peer1 to the channel—-------------"

export CORE_PEER_LOCALMSPID=treasuryMSP 
export CORE_PEER_ADDRESS=localhost:9053
export CORE_PEER_TLS_ROOTCERT_FILE=${PWD}/organizations/peerOrganizations/treasury.scholar.com/peers/peer1.treasury.scholar.com/tls/ca.crt
export CORE_PEER_MSPCONFIGPATH=${PWD}/organizations/peerOrganizations/treasury.scholar.com/users/Admin@treasury.scholar.com/msp
...
jq '.channel_group.groups.Application.groups.universityMSP.values += {"AnchorPeers":{"mod_policy": "Admins","value":{"anchor_peers": [{"host": "peer0.university.scholar.com","port": 8051}]},"version": "0"}}' config_copy.json > modified_config.json
...
jq '.channel_group.groups.Application.groups.auditorMSP.values += {"AnchorPeers":{"mod_policy": "Admins","value":{"anchor_peers": [{"host": "peer0.auditor.scholar.com","port": 11051}]},"version": "0"}}' config_copy.json > modified_config.json
...
peer lifecycle chaincode checkcommitreadiness --channelID $CHANNEL_NAME --name Scholar --version 1.0 --sequence 1 --tls --cafile $ORDERER_CA --output json

peer lifecycle chaincode commit -o localhost:7050 --ordererTLSHostnameOverride orderer.scholar.com --channelID $CHANNEL_NAME --name Scholar --version 1.0 --sequence 1 --tls --cafile $ORDERER_CA --peerAddresses localhost:7051 --tlsRootCertFiles $scholarshipDepartment_PEER_TLSROOTCERT --peerAddresses localhost:9051 --tlsRootCertFiles $treasury_PEER_TLSROOTCERT --peerAddresses localhost:8051 --tlsRootCertFiles $university_PEER_TLSROOTCERT --peerAddresses localhost:11051 --tlsRootCertFiles $auditor_PEER_TLSROOTCERT

sleep 1

peer lifecycle chaincode querycommitted --channelID $CHANNEL_NAME --name Scholar --cafile $ORDERER_CA
